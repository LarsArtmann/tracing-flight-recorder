...
| Dump metadata envelope           | 🟢 `FULLY_FUNCTIONAL` | `FlightRecorderDump { schema_version, captured_at, crate_version, event_count, trigger_reason, events }` via `dump_envelope`, `dump_envelope_to_json` (compact) / `dump_envelope_to_json_pretty`, `dump_envelope_to_file` / `_pretty`, `dump_with_retention_envelope`; tested `dump_envelope_to_file_writes_metadata_and_events`, `dump_envelope_round_trips_through_json`, `dump_with_retention_envelope_writes_envelope_format`, `envelope_pretty_variant_indents_and_round_trips` |

## Observability

| Feature                              | Status                | Notes                                                                                |
| ------------------------------------ | --------------------- | ------------------------------------------------------------------------------------ |
| `on_dump` callback                   | 🟢 `FULLY_FUNCTIONAL` | `FlightRecorder::with_on_dump(callback)` fires a `DumpEvent` (path, bytes_written, duration, trigger_reason, source) after every file-writing dump — manual, retention, and automatic trigger dumps. `DumpSource` distinguishes `Manual` vs `Trigger`. Panicking callbacks are contained via `catch_unwind` (`src/layer.rs`, `src/capture.rs`); tested `on_dump_fires_for_manual_file_dump`, `on_dump_fires_for_trigger_dump_with_trigger_source`, `on_dump_callback_panic_is_contained` |
