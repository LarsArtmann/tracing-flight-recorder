...
| ------------------------------------ | --------------------- | ------------------------------------------------------------------------------------ |
| Bounded ring buffer with eviction    | 🟢 `FULLY_FUNCTIONAL` | `FlightRecorder::push` evicts oldest when at capacity (`FlightRecorder::push`); tested `ring_buffer_evicts_oldest_at_capacity`, `capacity_one_evicts_immediately` |
| Insertion-order snapshots            | 🟢 `FULLY_FUNCTIONAL` | `snapshot()` returns oldest-first (`FlightRecorder::snapshot`); tested `snapshot_returns_events_in_insertion_order` |
...
| Feature                              | Status                | Notes                                                                                |
| ------------------------------------ | --------------------- | ------------------------------------------------------------------------------------ |
| `tracing_subscriber::Layer` impl     | 🟢 `FULLY_FUNCTIONAL` | `FlightRecorderLayer::on_event` → `CapturedEvent::from_event` (`FlightRecorderLayer::on_event`); tested end-to-end `layer_captures_real_tracing_events` |
...
| Structured field capture (all types) | 🟢 `FULLY_FUNCTIONAL` | `FieldVisitor` handles str/bool/i64/u64/f64/i128/u128/debug/error (`FieldVisitor`); tested `layer_captures_structured_fields_from_real_events` |
...
| Feature                         | Status                | Notes                                                                                |
| ------------------------------- | --------------------- | ------------------------------------------------------------------------------------ |
| JSON serialization              | 🟢 `FULLY_FUNCTIONAL` | `dump_to_json()` → compact JSON array; `dump_to_json_pretty()` → indented (`FlightRecorder::dump_to_json`); tested `dump_to_json_produces_valid_json_array`, `dump_to_json_is_compact_and_pretty_variant_indents` |
